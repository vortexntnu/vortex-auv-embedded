...
static led_software_mode_t s_software_mode = LED_SW_KILLSWITCH;

// physical LED indices for b3,b2,b1,b0
static uint8_t s_led_index_for_bit[4] = {3,2,1,0};

// Software mode uses physical LED index 5.
// LED 4 is broken and intentionally unused.
#define SOFTWARE_MODE_LED_INDEX  (4u)
...
static void set_led(uint8_t idx, rgb_t c)
{
...
static void show_all_off(void)
{
    for (unsigned k=0; k<4; k++) {
        set_led(s_led_index_for_bit[k], C_OFF);
    }
}

static void show_software_mode_led(void)
{
    switch (s_software_mode)
    {
        case LED_SW_AUTONOMOUS:
            set_led(SOFTWARE_MODE_LED_INDEX, C_BLU);
            break;

        case LED_SW_MANUAL:
            set_led(SOFTWARE_MODE_LED_INDEX, C_YEL);
            break;

        case LED_SW_REFERENCE:
            set_led(SOFTWARE_MODE_LED_INDEX, C_GRN);
            break;

        case LED_SW_KILLSWITCH:
        default:
            set_led(SOFTWARE_MODE_LED_INDEX, C_RED);
            break;
    }
}
...
static void render_current_frame_to_led_buffer(void)
{
    ...
    if (s_phase == PHASE_OFF) {
        ...
        // Software mode is constant and should not blink with alarm frames.
        show_software_mode_led();
        return;
    }
    ...
    // Software mode is always shown on physical LED 5.
    show_software_mode_led();
}
...
{
    for (unsigned i=0; i<LED_LOGIC_MAX_FAULTS; i++) {
        if (s_faults[i].active && s_faults[i].subsystem_id4 == subsystem_id4) {
            return (int)i;
        }
    }
    return -1;
...
{
    for (unsigned i=0; i<LED_LOGIC_MAX_FAULTS; i++) {
        if (!s_faults[i].active) return (int)i;
    }
    return -1;
}
...
void led_logic_init(void)
{
    ...
    s_software_mode = LED_SW_KILLSWITCH;
    ...
    for (unsigned i = 0; i < (sizeof(startup_faults) / sizeof(startup_faults[0])); i++) {
        const uint8_t id = startup_faults[i];
        const unsigned slot = i;
