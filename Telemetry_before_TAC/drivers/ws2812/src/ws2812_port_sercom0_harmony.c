...
#  define WS2812_RESET_PAD_BYTES        (24u)     // ~80us low @ ~2.4 MHz SPI
#endif

#ifndef WS2812_PRE_PAD_BYTES
#  define WS2812_PRE_PAD_BYTES          (24u)     // ~80us low @ ~2.4 MHz SPI
#endif

// Debug: count how many times the DMAC TX complete callback runs
//static volatile uint32_t g_dmac_tx_done_count = 0;
//uint32_t ws_led_dbg_get_isr_count(void) { return g_dmac_tx_done_count; }
...
// We already append a long zero pad, so the latch is satisfied.
// Just invoke the completion immediately.
