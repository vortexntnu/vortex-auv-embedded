...
static void uart_write_voltages(const uint16_t cell_mV[6]) {
    char line[96];
    int len = snprintf(line, sizeof(line), "%u,%u,%u,%u,%u,%u\r\n",
                       (unsigned int)cell_mV[0], (unsigned int)cell_mV[1],
                       (unsigned int)cell_mV[2], (unsigned int)cell_mV[3],
                       (unsigned int)cell_mV[4], (unsigned int)cell_mV[5]);
...
static void uart_write_alert_ssa(void) {
    ...
    len = snprintf(line, sizeof(line),
                   ...
                   (unsigned int)alarm, (unsigned int)ssa, (unsigned int)ssb,
                   (unsigned int)ssc);
...
static uint8_t uart_write_alert_pfa(void) {
    uint16_t alarm = 0U;
    uint16_t ssa = 0U;
    uint16_t ssb = 0U;
    uint16_t ssc = 0U;
    ...
    len = snprintf(line, sizeof(line),
                   ...
                   (unsigned int)alarm, (unsigned int)ssa, (unsigned int)ssb,
                   (unsigned int)ssc, (unsigned int)fet);
...
void spi_write_probe_step(void) {
    ...
    len = snprintf(line, sizeof(line), "wr,0x%02X,%s\r\n", (unsigned int)tx,
                   ok ? "ok" : "fail");
...
bool read_data_memory(uint16_t address, uint8_t* data, uint8_t length) {
    ...
    } while ((echo[0] == 0xFFU && echo[1] == 0xFFU) || (echo[0] != subcmd[0]) ||
             (echo[1] != subcmd[1]));
...
static void uart_write_fet_options_and_mfg_status_init(void) {
    ...
    mfg_status_init =
        (uint16_t)mfg_status_bytes[0] | ((uint16_t)mfg_status_bytes[1] << 8);

    len = snprintf(line, sizeof(line), "fet_opt=0x%02X,mfg_init=0x%04X\r\n",
                   (unsigned int)fet_options, (unsigned int)mfg_status_init);
...
static void uart_write_battery_status(void) {
    ...
    len = snprintf(
        ...
        mode, (unsigned int)chg_on, (unsigned int)pchg_on, (unsigned int)dsg_on,
        (unsigned int)ctrl, (unsigned int)batt, (unsigned int)fet);
...
uint8_t voltage_test_step(void) {
    ...
    uint8_t fet = uart_write_alert_pfa();
    LED_Y_Toggle();
    return fet;
}
...
void can_scope_test_step(void) {
    ...
    if (CAN_TryRead(&rx_id, &rx_len, rx_data)) {
        len = snprintf(line, sizeof(line), "rx,0x%03X,%u,0x%02X,0x%02X\r\n",
                       (unsigned int)rx_id, (unsigned int)rx_len,
