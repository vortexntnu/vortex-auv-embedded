...
#define MS_ADDR 0x76
#define MS5837_RESET_DELAY_MS       (3U)
...
static int reset_seq() {
    ...
    if (!SERCOM1_I2C_Write(MS_ADDR, &cmd_reset, 1)) {
        return -1;
    }
    if (!ms5837_wait_i2c_complete()) {
        return -1;
    }
...
static int prom_read_seq(struct ms5837_t* s) {
    ...
    for (int i = 0; i < 8; i++) {
        uint8_t rx[2];
        if (!SERCOM1_I2C_WriteRead(MS_ADDR, &cmd_prom_read, 1, rx, 2)) {
            return -1;
        }
        if (!ms5837_wait_i2c_complete()) {
            return -1;
...
static void ms5837_i2c_cb(uintptr_t context)
{
    ...
    s->i2c_ok = (SERCOM1_I2C_ErrorGet() == SERCOM_I2C_ERROR_NONE);
    s->i2c_done = true;
}
...
static bool ms5837_write_cmd(struct ms5837_t* s, uint8_t cmd)
{
    ...
    if (!SERCOM1_I2C_Write(MS_ADDR, s->tx, 1)) {
        return false;
    };
    TC0_TimerStart();
    return true;
...
static bool ms5837_read_adc24(struct ms5837_t* s)
{
    ...
    s->tx[0] = CMD_ADC_READ;
    rdy_to_sample = false;
    return SERCOM1_I2C_WriteRead(MS_ADDR, s->tx, 1, s->rx, 3);
}
...
int8_t ms5837_init(struct ms5837_t* s) {
    ...
    SERCOM1_I2C_CallbackRegister(ms5837_i2c_cb, (uintptr_t)s);
