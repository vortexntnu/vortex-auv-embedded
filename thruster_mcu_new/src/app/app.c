...
static struct pwm_output thrusters[8] = {
    {PWM_TCC, 2, 0, TCC2_PERIOD, 1000, 2000, 1500, THRUSTER_PWM_PERIOD_US, 1500,
     1500},  // TH1 -> TCC2_CC0
    {PWM_TCC, 2, 1, TCC2_PERIOD, 1000, 2000, 1500, THRUSTER_PWM_PERIOD_US, 1500,
     1500},  // TH2 -> TCC2_CC1
    {PWM_TCC, 1, 0, TCC1_PERIOD, 1000, 2000, 1500, THRUSTER_PWM_PERIOD_US, 1500,
     1500},  // TH3 -> TCC1_CC0
    {PWM_TCC, 1, 1, TCC1_PERIOD, 1000, 2000, 1500, THRUSTER_PWM_PERIOD_US, 1500,
     1500},  // TH4 -> TCC1_CC1
    {PWM_TCC, 0, 1, TCC0_PERIOD, 1000, 2000, 1500, THRUSTER_PWM_PERIOD_US, 1500,
     1500},  // TH5 -> TCC0_CC1
    {PWM_TCC, 0, 0, TCC0_PERIOD, 1000, 2000, 1500, THRUSTER_PWM_PERIOD_US, 1500,
     1500},  // TH6 -> TCC0_CC0
    {PWM_TCC, 0, 3, TCC0_PERIOD, 1000, 2000, 1500, THRUSTER_PWM_PERIOD_US, 1500,
     1500},  // TH7 -> TCC0_CC3
    {PWM_TCC, 0, 2, TCC0_PERIOD, 1000, 2000, 1500, THRUSTER_PWM_PERIOD_US, 1500,
     1500}  // TH8 -> TCC0_CC2
};

static struct pwm_output lights[1] = {{MPWM_TC, 3, 1, TC3_PERIOD, 1100, 1900,
                                       1100, LIGHT_PWM_PERIOD_US,
                                       1100}};  // TC3_CC1
...
// FOR TESTING
void test_thrusters(const uint8_t* thruster_indices,
                    size_t count,
                    uint16_t max_us,
                    uint16_t min_us,
                    uint16_t step_us,
                    uint32_t step_delay_ms);
void test_thrusters_seq(const uint8_t* thruster_indices,
                        size_t count,
                        uint16_t max_us,
                        uint16_t min_us,
                        uint16_t step_us,
                        uint16_t step_delay_ms);
void test_neutral_to_max(const uint8_t* thruster_indices,
                         size_t count,
                         uint16_t max_us,
                         uint32_t hold_ms);
void test_thrusters_split(const uint8_t* thruster_indices,
                          size_t count,
                          uint16_t max_us,
                          uint16_t min_us,
                          uint16_t step_us,
...
/**
 * @brief Sets PWM pulse widths for multiple PWM outputs (e.g Thrusters and/or
 * lights).
 *
 ...
 * @return The clamped value: low if value < low, high if value > high,
 * otherwise value
 */
 ...
 *
 * Routes the PWM write to the appropriate TCC peripheral based on instance
 * number.
 *
...
static void start_adc_conversion(RTC_TIMER32_INT_MASK intCause,
                                 uintptr_t context);
...
void app_init(void) {
    ...
    // SERCOM4_USART_Enable();
    // SERCOM4_USART_ReadCallbackRegister(uart_receive_callback,
    // (uintptr_t)NULL); SERCOM4_USART_Read(uart_header, UART_HEADER_SIZE);
    uart_bridge_init();
...
void app_task(void) {
    ...
    //         dispatch_hw_event(&hw_events.pgood_pending_mask,
    //         send_pgood_event);
    //     }
    //
    //     if (hw_events.killswitch_pending_mask) {
    //         dispatch_hw_event(&hw_events.killswitch_pending_mask,
    //         send_killswitch_event);
    //     }
    //
    //
...
bool uart_send_frame(uint8_t msg_id, const uint8_t* payload, uint8_t length) {
    ...
    return SERCOM4_USART_Write(uart_tx_frame, (size_t)(4U + length));
}
    ...
    switch (uart_rx_state) {
        case UART_STATE_WAIT_HEADER: {
            ...
            if (uart_header[0] != UART_START_BYTE) {
                ...
                SERCOM4_USART_Read(uart_header, UART_HEADER_SIZE);
                return;
            }
            ...
            if (uart_msg_len == 0U) {
                /* No payload: checksum is just MSG_ID ^ LENGTH ^ (no bytes) =
                 * MSG_ID ^ LENGTH. We still need to read the single checksum
                 * byte before validating. */
                uart_rx_state = UART_STATE_WAIT_PAYLOAD;
                SERCOM4_USART_Read(uart_payload, 1U); /* checksum only */
            } else {
                ...
                uart_rx_state = UART_STATE_WAIT_PAYLOAD;
                SERCOM4_USART_Read(uart_payload, (size_t)(uart_msg_len + 1U));
            }
            break;
        }
        ...
        case UART_STATE_WAIT_PAYLOAD: {
            ...
            uint8_t expected_checksum =
                compute_checksum(uart_msg_id, uart_msg_len, uart_payload);
...
static void log_current(void) {
    ...
    for (size_t i = 0; i < 8; i++) {
        ...
        //        printf("\nTH%u (AIN%u) raw=%u  V=%.4f  I=%.3f A PWM=%u
        //        us\r\n",
        //               imon_map[i].thruster,
        //               imon_map[i].ain,
        ...
        // printf("%u %.3f\r\n\r\n", (unsigned)thrusters[imon_map[i].thruster -
        // 1].current_pulse_us, I_out);
    }
...
static void set_pwm_outputs(const uint8_t* data,
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        // uint32_t ticks = us_to_ticks(outputs[i].period_ticks, pulse_us,
        // outputs[i].frame_us);
...
// void test_thrusters(const uint8_t *thruster_indices, size_t count, uint16_t
// max_us, uint16_t min_us, uint16_t step_us, uint32_t step_delay_ms) {
//     /* Beregn test_max og test_min per thruster */
//     uint16_t test_max[8];
//     uint16_t test_min[8];
//     for (size_t i = 0; i < count; i++) {
//         uint8_t idx = thruster_indices[i];
//         test_max[i] = clamp(max_us, thrusters[idx].neutral_us,
//         thrusters[idx].max_us); test_min[i] = clamp(min_us,
//         thrusters[idx].min_us,     thrusters[idx].neutral_us);
//     }
//
//     /* --- Neutral -> Max --- */
...
//             uint32_t ticks = us_to_ticks(th->period_ticks, clamped,
//             th->frame_us); tcc_write(th->instance, th->channel, ticks);
...
//         uint8_t idx = thruster_indices[i];
//         struct pwm_output *th = &thrusters[idx];
//         uint32_t ticks = us_to_ticks(th->period_ticks, th->neutral_us,
//         th->frame_us); tcc_write(th->instance, th->channel, ticks);
//         th->current_pulse_us = th->neutral_us;
//     }
// }
//
// void test_thrusters_seq(const uint8_t *thruster_indices, size_t count,
// uint16_t max_us, uint16_t min_us, uint16_t step_us, uint16_t step_delay_ms) {
//     for (size_t i = 0; i < count; i++) {
//         uint8_t idx = thruster_indices[i];
//         if (idx >= 8) {
//             continue;
...
//
//         /* Snap to neutral */
//         uint32_t neutral_ticks = us_to_ticks(th->period_ticks,
//         th->neutral_us, th->frame_us); tcc_write(th->instance, th->channel,
//         neutral_ticks); th->current_pulse_us = th->neutral_us;
//
//         /* --- Neutral -> Min --- */
...
// }
//
// void test_thrusters_split(const uint8_t *thruster_indices, size_t count,
// uint16_t max_us, uint16_t min_us, uint16_t step_us, uint32_t step_delay_ms) {
//         size_t first_count = (count < 4) ? count : 4;
//         test_thrusters(thruster_indices, first_count, max_us, min_us,
//         step_us, step_delay_ms);
//
//         if (count > 4) {
//             test_thrusters(thruster_indices + 4, count - 4, max_us, min_us,
//             step_us, step_delay_ms);
//         }
//
//
// }
//
// void test_neutral_to_max(const uint8_t *thruster_indices, size_t count,
// uint16_t max_us, uint32_t hold_ms) {
//     for (size_t i = 0; i < count; i++) {
//         uint8_t idx = thruster_indices[i];
//         struct pwm_output *th = &thrusters[idx];
//
...
//
//
//     //uint32_t ticks = us_to_ticks(TC3_PERIOD, pulse_us,
//     LIGHT_PWM_PERIOD_US);
//
...
static void can_receive_callback(uint8_t numberOfMessage, uintptr_t context) {
    ...
    if (((can_status & CAN_PSR_LEC_Msk) == CAN_ERROR_NONE) ||
        ...
        if (CAN1_MessageReceiveFifo(CAN_RX_FIFO_0, numberOfMessage,
                                    (CAN_RX_BUFFER*)rxFiFo0) == true) {
