...
static void watchdog_can_send(uint32_t can_id,
                              const uint8_t* data,
                              uint8_t len) {
...
int main(void) {
    ...
    MS5837_Init(&external_pressure_device);

    // Enable watchdog (integrated into decoder module)
    // - CAN IDs and default monitor IDs are defined in led_can_decode.h
    // - You can override alive-req CAN ID / monitor IDs later via the
    // led_can_watchdog_set_* API.
    ...
    while (true) {
        ...
        if (rxReady) {
            ...
            // Feed watchdog with every received frame (passive monitoring +
            // IM_GOOD decoding)
            led_can_watchdog_on_can_rx(rx_messageID, rx_message,
                                       rx_messageLength, millis());

            // Filter for LED-systems addresses
            if (rx_messageID >= 0x101u && rx_messageID <= 0x10Bu) {
                led_can_decode_and_update(rx_messageID, rx_message,
                                          rx_messageLength);
            ...
            printf("Timestamp: 0x%x ID: 0x%lx Length: 0x%x\r\n",
                   (unsigned)timestamp, (unsigned long)rx_messageID,
            ...
            // CAN_Send(LED_CMD_STDID, can_test_payload,
            // sizeof(can_test_payload));
        }
        ...
        /*if (rxReady)
        {
            ...
            if (rx_messageID == LED_CMD_STDID){
                led_set(rx_message[0], rx_message[1], rx_message[2],
        rx_message[3]); uncommitted_changes = true;
            }
        ...
        if ((uint32_t)(millis() - last_pressure_read) >= pressure_interval_ms) {
            float ext_pressure[4];
            if (MS5837_Read(&external_pressure_device)) {
                ext_pressure[0] =
                    MS5837_Pressure(&external_pressure_device, MS5837_PA);
