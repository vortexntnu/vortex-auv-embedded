...
#define LED_LOGIC_MAX_FRAMES (1u + LED_LOGIC_MAX_FAULTS*2u)
#endif
...
static void build_frames(void)
{
    ...
    for (unsigned i=0;i<LED_LOGIC_MAX_FAULTS;i++) {
        ...
        if (s_faults[i].has_detail && s_frame_count < LED_LOGIC_MAX_FRAMES) {
            s_frames[s_frame_count++] = (frame_t){
                .type=FRAME_DETAIL,
                .sev=LED_SEV_WARN,
                .mask4=mask4(s_faults[i].detail_mask4)
            };
        }
    }
                             ...
                             uint8_t detail_mask_4bit)
{
    ...
    s_faults[idx].detail_mask4 = mask4(detail_mask_4bit);
