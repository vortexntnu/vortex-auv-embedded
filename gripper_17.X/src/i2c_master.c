...
void SERCOM2_I2C_Initialize(void) {
     ...
     * master enable */
    SERCOM2_REGS->I2CM.SERCOM_CTRLA =
...
static bool SERCOM2_I2C_CalculateBaudValue(uint32_t srcClkFreq,
...
bool SERCOM2_I2C_TransferSetup(SERCOM_I2C_TRANSFER_SETUP* setup,
    ...
    if (SERCOM2_I2C_CalculateBaudValue(srcClkFreq, i2cClkSpeed, &baudValue) ==
        false) {
        return false;
    }
    ...
    SERCOM2_REGS->I2CM.SERCOM_CTRLA =
        ((SERCOM2_REGS->I2CM.SERCOM_CTRLA & ~SERCOM_I2CM_CTRLA_SPEED_Msk) |
...
static bool SERCOM2_I2C_XferSetup(uint16_t address,
                                  uint8_t* wrData,
                                  uint32_t wrLength,
                                  uint8_t* rdData,
                                  uint32_t rdLength,
...
bool SERCOM2_I2C_Read(uint16_t address, uint8_t* rdData, uint32_t rdLength) {
    return SERCOM2_I2C_XferSetup(address, NULL, 0, rdData, rdLength,
                                 I2C_TRANSFER_READ, false);
}

bool SERCOM2_I2C_Write(uint16_t address, uint8_t* wrData, uint32_t wrLength) {
    return SERCOM2_I2C_XferSetup(address, wrData, wrLength, NULL, 0,
...
bool SERCOM2_I2C_WriteRead(uint16_t address,
                           ...
                           uint8_t* rdData,
                           uint32_t rdLength) {
    return SERCOM2_I2C_XferSetup(address, wrData, wrLength, rdData, rdLength,
                                 I2C_TRANSFER_WRITE, false);
}

bool SERCOM2_I2C_IsBusy(void) {
    if ((sercom2I2CObj.state == SERCOM_I2C_STATE_IDLE) &&
        ((SERCOM2_REGS->I2CM.SERCOM_STATUS & SERCOM_I2CM_STATUS_BUSSTATE_Msk) ==
...
void __attribute__((used)) SERCOM2_Handler(void) {
    if (SERCOM2_REGS->I2CM.SERCOM_INTENSET != 0) {
        ...
        if ((SERCOM2_REGS->I2CM.SERCOM_STATUS &
        ...
        else if ((SERCOM2_REGS->I2CM.SERCOM_STATUS &
            ...
            switch (sercom2I2CObj.state) {
                case SERCOM_I2C_REINITIATE_TRANSFER:
                    ...
                    if (sercom2I2CObj.writeSize != 0) {
                        ...
                        SERCOM2_I2C_InitiateTransfer(sercom2I2CObj.address,
                                                     false);
                    } else {
                ...
                case SERCOM_I2C_STATE_TRANSFER_WRITE:
                    ...
                    if (sercom2I2CObj.writeCount == (sercom2I2CObj.writeSize)) {
                        if (sercom2I2CObj.readSize != 0) {
                             ...
                             * equal to 1*/
                            SERCOM2_REGS->I2CM.SERCOM_ADDR =
                                (sercom2I2CObj.address << 1) |
                                I2C_TRANSFER_READ;
                    ...
                    /* Write next byte */
                        ...
                        SERCOM2_REGS->I2CM.SERCOM_DATA =
                            sercom2I2CObj
                                .writeBuffer[sercom2I2CObj.writeCount++];
                    }
                ...
                case SERCOM_I2C_STATE_TRANSFER_READ:
                    ...
                    if (sercom2I2CObj.readCount ==
                        (sercom2I2CObj.readSize - 1)) {
                    ...
                    sercom2I2CObj.readBuffer[sercom2I2CObj.readCount++] =
                        SERCOM2_REGS->I2CM.SERCOM_DATA;
        ...
        /* Transfer Complete */
            ...
            while ((SERCOM2_REGS->I2CM.SERCOM_STATUS &
