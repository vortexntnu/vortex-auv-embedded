...
static void ad7606_set_channels(struct ad7606_device* device, struct ad7606_channel channels[8]){
	...
	for(int i = 0; i < 8; i++){
		...
		device->registers->channel_range[i/2].data &= ~((0x0F) << (4*(i%2)));
		device->registers->channel_range[i/2].data |= ((channels[i].range & 0x0F) << (4*(i%2)));
		device->registers->diagnostics_mux[i/2].data &= ~(0x07 << (3*(i%2)));
		device->registers->diagnostics_mux[i/2].data |= ((channels[i].mux_ctrl & 0x07) << (3*(i%2)));
	}
}
