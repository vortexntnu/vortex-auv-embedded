...
def test_fullstack_premade_random_drone_data_direction(config_path: Path, csv_path: Path, fullstack_perf_recorder, request) -> None:
    if not _ALL_CASES:
        pytest.skip(
...
@pytest.mark.slow
@pytest.mark.parametrize("config_path,csv_path", _param_cases(), ids=_param_ids())
def test_fullstack_premade_random_drone_data_position(config_path: Path, csv_path: Path, fullstack_perf_recorder, request) -> None:
    if not _ALL_CASES:
        pytest.skip(
            "No premade test data found in tests/test_data. "
            "Generate it first with: python test_data_generator.py -n 100"
    ...
    assert position_error is not None and math.isfinite(position_error), (
        f"Position error not finite: {position_error} (config={config_path.name})"
    )
    assert abs(position_error) <= 45.0 and position_error >= 0.0, (
        f"Position error out of bounds: {position_error} (config={config_path.name})"
    )