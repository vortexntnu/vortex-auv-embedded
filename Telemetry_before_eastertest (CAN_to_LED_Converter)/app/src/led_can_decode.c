...
// ============================================================================
// Decoder (existing, verified behavior) - minimal changes only
// ============================================================================
...
bool led_can_decode_and_update(uint32_t can_id, const uint8_t *data, uint8_t len)
{
    ...
    // Only thrusters use the full 8-bit active-detail bitmap.
    // All other legacy subsystem messages keep their existing low-4-bit behavior.
    const uint8_t detail_bits = (can_id == CAN_ID_THRUSTERS) ? parameter : mask4(parameter);
    const bool    has_detail  = (detail_bits != 0u);
    ...
    switch ((msg_type_t)type)
    {
        ...
        case MSG_WARN:
            led_logic_set_subsystem(LED_SEV_WARN, subsystem_id, has_detail, detail_bits);
            if (can_id == CAN_ID_PRESSURE) led_logic_set_pressure_ok(false);
            return true;

        case MSG_FAULT:
            led_logic_set_subsystem(LED_SEV_FAULT, subsystem_id, has_detail, detail_bits);
            if (can_id == CAN_ID_PRESSURE) led_logic_set_pressure_ok(false);
            return true;
...
// ============================================================================
// Integrated watchdog
// MCU subsystem now stores an active-set bitmap for details 1..4,
// and LED logic shows each active detail one by one.
// ============================================================================
...
// MCU alarm storage:
// index 1..4 corresponds to MCU detail numbers 1..4 on subsystem 3.
...
static uint8_t build_mcu_detail_bits(void)
{
    uint8_t bits = 0u;

    if (g_mcu_alarm_active[1]) bits |= (1u << 0); // detail 1
    if (g_mcu_alarm_active[2]) bits |= (1u << 1); // detail 2
    if (g_mcu_alarm_active[3]) bits |= (1u << 2); // detail 3
    if (g_mcu_alarm_active[4]) bits |= (1u << 3); // detail 4
...
{
    const uint8_t detail_bits = build_mcu_detail_bits();

    if (detail_bits != 0u) {
        // Subsystem 3 = MCU_POWER
        // LED logic will show each active MCU detail one by one as 1..4.
        led_logic_set_subsystem(LED_SEV_FAULT, 3u, true, detail_bits);
    } else {
...
static void apply_alarm_to_led(led_node_id_t node, bool alarm)
{
    ...
    // MCUs use subsystem 3 and details 1..4.
...
void led_can_watchdog_tick(uint32_t now_ms)
{
    ...
    for (int i = 1; i <= LED_NODE_COUNT; i++) {
        ...
        // Probe timeout => remain/go to ALARM
        if (n->probe_pending && ((int32_t)(now_ms - n->probe_deadline_ms) >= 0)) {
            n->probe_pending = false;
            n->state = WD_ALARM;
